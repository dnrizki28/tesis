...
keep hhid14 sanitasi minum grup_sanitasi
...
merge 1:m hhid14 using "$ifls5\b1_ks4"
...
bysort hhid14 : egen dairy_foods=max(dairy_source)
...
merge 1:m hhid14 using "`hhsize_exp'"
keep if _merge==3
drop _merge

keep hhid14 hh_food_exp hh_size
...
bysort hhid14: egen hh_nonfood=total(ks06)
bysort hhid14: keep if _n==1
merge 1:1 hhid14 using "`food_month'"
drop if _merge==1

egen hh_exp14=rowtotal(hh_food_exp_month hh_nonfood)
tempfile ks1_2_14
save "`ks1_2_14'"

gen exp_cap14=hh_exp14/hh_size
gen food_cap_month= hh_food_exp_month/hh_size
gen ratio_food=food_cap_month/ exp_cap14*100
keep hhid14 ratio_food
...
merge m:1 hhid14 using "`share_foodexp'"
keep if _merge==3
drop _merge
/*----------------------------------------------------------------------------*/
